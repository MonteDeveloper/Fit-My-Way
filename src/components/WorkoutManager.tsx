...
export const WorkoutManager: React.FC<WorkoutManagerProps> = ({
  ...
  const handleSave = async () => {
    ...
    const saved = await db
      .getWorkouts()
      .then((res) => res.find((w) => w.id === finalWorkout.id));
    if (saved) {
  ...
  const addSet = (exerciseIdx: number) => {
    ...
    const lastSet =
      newExs[exerciseIdx].sets[newExs[exerciseIdx].sets.length - 1];
        ...
        {mode === "detail" && selectedWorkout && (
          ...
          >
            {(() => {
              ...
              const ct =
                selectedWorkout.coverTransform || DEFAULT_COVER_TRANSFORM;
              const isResumable =
                    ...
                    <div className="relative min-h-[16rem] w-full overflow-hidden bg-black">
                      ...
                      <div className="absolute bottom-0 left-0 p-6 text-white z-10 w-full">
                        ...
                        <h1
                          ...
                          className={`text-3xl font-black mb-2 leading-tight cursor-pointer ${
                            isTitleExpanded ? "whitespace-normal" : "truncate"
                          }`}
                          title="Tap to expand"
                        >
                    ...
                    <div className="p-4 space-y-6 relative z-10 bg-white dark:bg-dark rounded-t-3xl -mt-4 min-h-[calc(100vh-16rem)]">
                      ...
                      <div className="space-y-3">
                        {isResumable && (
                          <button
                            onClick={() =>
                              onStartWorkout(selectedWorkout, true)
                            }
                      ...
                      <div className="space-y-3 pb-8">
                        ...
                        {selectedWorkout.exercises.map((ex, idx) => {
                          const exDef = allExercises.find(
                            (e) => e.id === ex.exerciseId
                          );
                          const tImg = exDef?.imageTransform || {
                            x: 0,
                            y: 0,
                            scale: 1,
                          };
                          ...
                          return (
                            <div key={idx}>
                              <div
                                onClick={() =>
                                  onNavigateToExercise &&
                                  onNavigateToExercise(ex.exerciseId)
                                }
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-4 pb-8">
                  {editingWorkout.exercises?.map((wEx, idx) => {
                    const exDef = allExercises.find(
                      (e) => e.id === wEx.exerciseId
                    );
                    const isLastItem =
                      idx === (editingWorkout.exercises?.length || 0) - 1;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="space-y-2 mb-3">
                          ...
                          {(Array.isArray(wEx.sets) ? wEx.sets : []).map(
                            (set, sIdx) => (
                              <div
                                key={set.id}
                                className="grid grid-cols-[30px_1fr_1fr_1fr_30px] gap-2 items-center"
                              >
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div className="relative w-16">
                                  <input
                                    ...
                                    className="w-full bg-gray-50 dark:bg-slate-900 p-1 rounded text-center text-xs"
                                    value={
                                      wEx.restAfterExercise === 0
                                        ? ""
                                        : wEx.restAfterExercise
                                    }
                                    onChange={(e) => {
                                      const newExs = [
                                        ...(editingWorkout.exercises || []),
                                      ...
                                      const val = e.target.value;
                                      (newExs[idx] as any).restAfterExercise =
                                        val;
                                      setEditingWorkout({
                                        ...editingWorkout,
                                        exercises: newExs,
          ...
          {showCoverEditor && (
            ...
            >
              <div className="p-4 border-b border-gray-100 dark:border-slate-800 grid grid-cols-[40px_1fr_40px] items-center shadow-sm flex-none">
                ...
                <h3 className="font-bold text-lg text-center">{t.editCover}</h3>
                <div />
              </div>
                  ...
                  <div className="flex items-center gap-3 pt-3 justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-500 w-12 text-center font-mono">
                        {coverTransform.scale.toFixed(1)}x
                      </span>
                      <button
                        onClick={() =>
                          setCoverTransform((prev) => ({
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-4 bg-gray-200 dark:bg-slate-700 rounded-xl hover:bg-gray-300 dark:hover:bg-slate-600"
                      type="button"
                    >
                      <Clipboard size={20} />
                    </button>
        ...
        <AnimatePresence>
          {showAddExModal && (
            <motion.div
              key="addExModal"
              {...subModalVariants}
              className="fixed inset-0 z-[70] bg-white dark:bg-dark flex flex-col h-full"
            >
              {/* FIXED HEADER - Sub Modal: Back Left */}
              <div className="flex-none p-4 border-b border-gray-100 dark:border-slate-800 grid grid-cols-[40px_1fr_auto] items-center bg-white/95 dark:bg-dark/95 backdrop-blur-md z-10 shadow-sm">
                <button
                ...
                <button
                  ...
                  className="px-4 py-2 bg-primary text-white rounded-full font-bold disabled:opacity-50 flex-shrink-0 ml-2"
                >
                  {t.add} ({exModalSelected.length})
                </button>
              </div>

              {/* FIXED SEARCH & FILTERS - Swapped Order & Added spacing */}
              <div className="flex-none p-4 space-y-3 bg-white dark:bg-dark border-b border-gray-100 dark:border-slate-800 z-10">
                ...
                <div className="flex gap-2 overflow-x-auto scrollbar-hide snap-x pb-1">
                  <button
                    ...
                    className={`snap-start px-3 py-1 rounded-full text-sm font-medium whitespace-nowrap flex-shrink-0 ${
                      exModalFilters.length === 0
                  ...
                  {MUSCLE_GROUPS.map((m) => {
                    ...
                    return (
                      <button
                        ...
                        className={`snap-start px-3 py-1 rounded-full text-sm font-medium whitespace-nowrap transition-colors flex-shrink-0 ${
                          isActive
                            ? "bg-primary text-white shadow-sm"
                            : "bg-gray-100 dark:bg-slate-800 text-gray-500 border border-transparent"
                        }`}
                      >
                ...
                {allExercises
                  .filter((e) => {
                    const matchesSearch = e.name
                      .toLowerCase()
                      .includes(exModalSearch.toLowerCase());
                    const matchesFilter =
                      exModalFilters.length > 0
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer ${
                          isSelected
                            ? "bg-indigo-50 dark:bg-indigo-900/20 border-primary"
                            : "bg-white dark:bg-slate-800 border-gray-100 dark:border-slate-700 hover:border-primary/50"
                        }`}
                      >
                        ...
                        <div className="flex-1 min-w-0 text-left">
                          ...
                          <div className="text-xs text-gray-500 truncate">
                            {e.muscleGroups
                              .map(
                                (m) => tMuscles[m as keyof typeof tMuscles] || m
                              )
                              .join(", ")}
                          </div>
                        </div>
              ...
              >
                ...
                <p className="text-gray-500 dark:text-gray-400 mb-6">
                  ...
                  <span className="font-bold">
                    "{workouts.find((w) => w.id === workoutToDelete)?.name}"
                  </span>
                  ? {tCommon.cannotUndo}
                </p>
